...
class InputMentions extends PureComponent {
  ...
  getHotWordOptions = () => {
    ...
    this.props.hotWordList &&
      this.props.hotWordList.forEach((item) => {
        item.hotWord &&
          options.push(
            <Mentions.Option key Option item={item.id} value={item.hotWord.substr(1)}>
              {item.hotWord}
            </Mentions.Option>,
          );
      });
  ...
  render() {
    ...
    let normalHeight = textSize('1.1em', '', changeText, wrapperWidth, 'normal').height;
    let nowrapHeight = textSize('1.1em', '', changeText, document.body.clientWidth, 'nowrap')
      .height;
    ...
    return (
      <>
        <Mentions
          ...
          style={style}
          prefix={optionPrefix}
          validateSearch={(text, props) => optionPrefix.some((item) => changeText.endsWith(item))}
        >
